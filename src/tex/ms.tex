...
Lastly, we combine equations \ref{eq: z log skew} and \ref{eq: sfr1} to form a full metallicity specific starformation rate density as described in equation \ref{eq: total sfrd}.
...
We fit for the following nine free parameters $\alpha, \mu_0, \mu_z, \omega_0, \omega_z$, which govern the metallicity dependence and $a,b, c$ and $d$, which set the overall starformation rate.
...
In Figure \ref{fig: fit SFRD} we show our fiducial \SFRDzZ model at different redshifts and metallicities. For clarity, 
...
\centering
\script{SFR_z.py}
\includegraphics[width=0.49\textwidth]{figures/SFR_redshift.pdf}
...
We consider variations of each of the parameters that determine the shape of the cosmic metallicity distribution \dpdZ (left two columns of Table \ref{tab: fit params}) and two variations of the \SFRDz, where we vary the complete shape of the \SFRDz (i.e. all four parameters) at once to match the values used in \cite{Madau+2017} and \cite{Neijssel2019} respectively. To determine the range of parameters that is reasonably allowed by observations, we compare the \SFRDzZ in the metallicity-redshift plane to the high and low metallicity extreme from \cite{Chruslinska+2021} by eye for every variation. \\
...
We fit our analytical expression for \SFRDzZ to the starforming-gas in the TNG100 simulation, and provide the best fit parameters in Table \ref{tab: fit params}. We show that our model captures the shape and general behaviour of the cosmological simulations well (Figure \ref{fig: fit SFRD}). 
...
We systematically vary all five parameters ($\alpha, \mu_0, \mu_z, \omega_0 and \omega_z$) that shape the cosmic metallicity distribution \dpdZ, and explore two addition variations of \SFRDz \citep[following][respectively]{Madau+2017,Neijssel+2019}. 
...
The flexibility of the model presented in this work can capture the large uncertainties that remain in the shape and normalisation of the metallicity dependent cosmic star formation history. As such, we anticipate that it will be a helpful tool in other works that present \SFRDzZ dependent rates. 

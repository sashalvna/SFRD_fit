...
\caption{Our fiducial \SFRDzZ model, adopting the best fitting parameters (listed on the top right) to fit the TNG100 simulation. 
The contours in the top panel range from $10^{-7} - 10^{-2} \Msun \yr^{-1}\Mpc^{-3}$. 
Fit for SFRD shown against TNG and Neijssel.  \label{fig: fit SFRD}}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Our best fitting parameters are listed in Table \ref{tab: fit params}. With these fit parameters, $\chi^2(z_j)$ is smaller than $2\cdot 10^{-4}$ at any given redshift. From now on, we will refer to the \SFRDzZ with the parameters listed in Table \ref{tab: fit params} as our fiducial \SFRDzZ model. 
...
In the top panel we show the general trend of the \SFRDzZ linear in lookback time. 

Main results:

1) We did a pretty good job at fitting the data. 
In general, the skewed-log normal metallicity distribution does a good job at capturing the metallicity dependent star formation in TNG. 
We did miss some features like the almoast dubble peak at redshift 2, 

Adopting a skewed-lognormal distribution furthermore allows for a tail of low metallicity star formation. To clarify, we compare to the phenomenological model from \cite{Neijssel+2019} in dotted grey. 


0) the starformation rate peaks at metallicities above $10^{-2}$ for all redshifts out to at least $z=6$
most star formation happens at metallicities above $Z \geq 10^{-2}$
...
% Things to say about this plot: 
We recognise some of the features discussed in \cite{Pakmor+2022}
We make a comparison to Neijssel fiducial: their model is extremely narrow, while the metallicity in the TNG is more broad

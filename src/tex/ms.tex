...
Two more features are visible in the stable channel at the high mass end for almost all \SFRDzZ (a knee at $\Mbheen\approx35\Msun$ and another bump at $\Mbheen\approx45\Msun$). Athough the locations of these features are constant, these features disappear for variations of  the metallicity-dependent cosmic star formation history where the rate of high mass BHs is suppressed (e.g., dashed lines in the top panels of Fig. \ref{fig: stable mass dists}).  
...
%
\cite{Broekgaarden+2021b} furthermore showed that variations in the \SFRDzZ can induce variations in the rate that are off the same order of magnitude as the variations caused by uncertainties in the binary physics. In their Fig. 4, they show the probability distribution of chirp masses for BBHs. Although features can disappear when \SFRDzZ prohibits the formation of certain (typically higher) mass BHs, the \textit{location} of features remains the same
%
